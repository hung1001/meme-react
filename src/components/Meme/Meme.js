...
const Meme = () => {
  ...
  const onClipboardSuccess = (event) => {
    ...
    let src = event.trigger.parentNode.previousElementSibling.previousElementSibling.src;
    let id = event.trigger.parentNode.id;
    setTooltip(id);
    setHistoryCopy(arr => {
      const icon = { value, src };
      let isDuplicate = arr.some(item => item.value === icon.value);
      return isDuplicate ? [...arr] : [icon, ...arr];
    });
