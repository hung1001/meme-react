...
import { DATA_URL, LOCALES } from 'utils/constants';

import { fetchJson, getTimeUpdated, setLocalStorage, getLocalStorage } from 'utils/helpers';
...
const Meme = () => {
  ...
  const [imgLang, setImgLang] = useState(() => {
    ...
    return (
      (getLang !== null && LOCALES.filter((locale) => locale.lang === getLang)[0].img) ||
      LOCALES[0].img
    );
  });
  ...
  const renderIcon = (item, index) => {
    ...
    const renderSearchKey = (key, value) => {
      ...
      let term = key
        .toLowerCase()
        .search(value.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&').toLowerCase());
          ...
          <div className="Meme__item lift" style={{ zIndex: tooltip === `icon-${index + 1}` && 1 }}>
            ...
            {tooltip === `icon-${index + 1}` && (
              <span className="tooltip show bs-tooltip-bottom">
                <span className="tooltip-inner" role="tooltip">
                  {t('meme.copied')}
                </span>
  ...
  const handleSearch = (event) => {
    ...
    const filterIcons = data.emoticons.filter(
      (item) => item.key.toLowerCase().indexOf(searchQuery) !== -1,
    );
  ...
  const renderHistoryCopy = (data) => {
    return data.map((icon, index) => (
      <div className="icon" key={index}>
        ...
        <span
          className="remove"
          title={t('meme.history_copy.remove')}
          onClick={removeHistoryIcon(index)}
        >
          ×
        </span>
      </div>
  ...
  if (!fetchDone) return <Loading />;

  return (
    <div className="Meme">
      <ChangeLanguage onChangeLang={handleChangeLang} currentLang={imgLang} />

      <section className="row align-items-center mt-5 mt-md-4">
        <div className="col-md">
          <h2 className="Meme__title mb-2 mb-md-0">
            {t('meme.title')}{' '}
            <a href="https://chatpp.thangtd.com/" target="_blank" rel="noopener noreferrer">
              {t('meme.chat++')}
            </a>
          </h2>
        </div>
      ...
      <section className="row align-items-center my-4 py-1">
        <div className="col-md-5">
          <div className="Meme__updated mb-2 mb-md-0">
            {t('meme.last_updated')} <b>{getTimeUpdated(data)}</b>, {data.emoticons.length}{' '}
            {data.emoticons.length > 2 ? t('meme.icons') : t('meme.icon')}
          </div>
        </div>
      ...
      <section className="Meme__search">
        <input
          ...
          type="search"
          placeholder={t('meme.input_placeholder')}
          onChange={handleSearch}
        />
        {query !== '' && (
          <p className="Meme__search-query">
            {t('meme.search.label')} <b>{query}</b>, {emo.length}{' '}
            {emo.length > 2 ? t('meme.search.results') : t('meme.search.result')}
          </p>
        )}
      </section>
      ...
      <footer className="Meme__footer">
        <p className="mb-0 mr-1">
          {t('meme.footer.made_by')}{' '}
          <a href="http://fb.com/100003827888172">{t('meme.footer.me')}</a> {t('meme.footer.with')}{' '}
          <MdHeart className="mt-n1" fontSize="20px" color="red" beat={true} />
        </p>
          ...
          <DropdownMenu className="Meme__dropdown-menu">
            {historyCopy.length === 0 ? (
              ...
              <div className="icons">
                <div
                  className="clear"
                  title={t('meme.history_copy.remove_all')}
                  onClick={() => setHistoryCopy([])}
                >
                  <svg
                    version="1.1"
                    id="Capa_1"
